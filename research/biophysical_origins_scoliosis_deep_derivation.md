...
This manuscript presents a rigorous theoretical derivation of Adolescent Idiopathic Scoliosis (AIS), transitioning beyond correlative observations to a first-principles causal mechanism. By resolving the "Gravitational Paradox," we demonstrate that biological growth is an active modification of the local spacetime metric requiring immense metabolic flux. We model the human spine as a growing Cosserat rod under active delayed feedback control. During the "Inflationary Epoch" of adolescence ($\dot{L} \gg 0$), the rapid volumetric expansion outpaces the informational bandwidth of the neural body schema, generating a spatiotemporal instability quantified by the dimensionless Scoliosis Number: $\mathcal{S}_{co} = \frac{K \dot{L} \tau}{B}$. Finally, we introduce "Spinal Jetlag"—a circadian desynchronization between bilateral growth plates—as the explicit Twist-Bend Coupling operator that converts planar sagittal buckling into the characteristic 3D helical deformity.
...
### Experiment 2: Jetlagged Torsion Induction

**Hypothesis:** Inducing a circadian phase shift between the left and right somites forces the symmetric spine to buckle into a chiral torsion.

**Protocol:**
1.  **Subject:** Zebrafish expressing the `Tg(per2:luc)` reporter line.
2.  **Intervention:** Chamber design utilizing split-tank illumination where the left and right sides are entrained to a 4-hour phase difference (Phase 0 vs Phase +4h).
3.  **Observation:** Measure vertebral aspect ratio and axial rotation over development.

**Prediction:**
Zebrafish subjected to split-illumination will predictably develop a specific 3D torsion proportional to the phase differential, mathematically mirroring human scoliotic progression and explicitly verifying the Twist-Bend Coupling operator.
