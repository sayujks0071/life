...
## A) Executive Summary
The project strategy has pivoted from a *Nature* submission to the *Spine* journal, reframing the core thesis as "A computational framework predicting adolescent scoliosis onset." Phase 1 (Computational Framework) is 75% complete, with the core "Energy Deficit" model, Rescue Cliff, and Spinal Jetlag simulations validated. We are now transitioning to Phase 2 (Clinical Validation) to explicitly map model outputs against published epidemiological data such as Peak Height Velocity (PHV), Lenke classifications, and sexual dimorphism prevalence. The manuscript will be reformatted to a standard IMRaD structure.

## B) Current State (Milestones Checklist)
**What's Done (Evidence: `docs/spine_submission_roadmap.md`, `outputs/`):**
- [x] **Core Model:** "Energy Deficit" bifurcation established (`scripts/experiments/experiment_energy_deficit_window.py`).
- [x] **Rescue Cliff Validation:** Validated anisotropy cliff at ~2.4 (`scripts/experiment_anisotropy_rescue.py`).
- [x] **Spinal Jetlag:** Circadian modulation of curvature demonstrated (`scripts/experiment_spinal_jetlag.py`).
...
**Target Submission Date:** 2026-04-06 (14 days remaining)
...
| **Lenke Classes** | `experiment_lenke_classes.py` | Spatial deficit $D(s)$ | Predicts specific scoliotic curves based on deficit localization. | ✅ Validated (Toy D) |
...
## F) Recommendations (Toy Models & Validation)
