...
### 18. Bioelectric Counter-Curvature Loop
* **Path**: `Collagen Strain` $\rightarrow$ `Piezoelectric Current` $\rightarrow$ `Galvanotaxis` $\rightarrow$ `Gain (G_{mech})`
* **Mechanism**: Mechanical deformation of bone collagen and dense connective tissue arrays generates a macroscopic bioelectric field (piezoelectricity). This field acts as an "Information Bus", rapidly directing morphogen transport (electrophoresis) and cellular growth (electrotaxis) to counteract the deformation.
* **Role**: **Instantaneous Global Gain**. This provides a rapid, tissue-wide corrective moment without the latency of neural transmission or slow diffusion.
> **Hypothesis**: The loss of this bioelectric field—due to hydration-induced short-circuiting (e.g., in microgravity swelling) or genetic collagen disorganization—renders the spine "electrically blind" to geometric deviations, dropping active feedback gain and causing scoliotic buckling.
