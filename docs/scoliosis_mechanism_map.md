...
### 18. POC5 Ciliary Retraction
* **Path**: `POC5` $\rightarrow$ `Centrosome Assembly` $\rightarrow$ `Ciliary Length` $\rightarrow$ `Gain (G_{mech})`
* **Mechanism**: POC5 is a centriolar protein required for centrosome assembly and ciliogenesis. The AIS-associated POC5 variant (e.g., POC5^A429V) exhibits altered subcellular localization and impaired protein-protein interactions, resulting in abnormal centrosome assembly and retraction of primary cilia.
* **Role**: **Loss of Sensory Antenna**. Defective ciliogenesis and ciliary retraction impair the primary cilium's function as a mechanosensory "antenna", leading to a loss of mechanotransductive gain ($G_{mech}$) and preventing the counter-curvature active control response from correctly adapting to spinal loading.
* **Evidence**: Hassan et al. (2019) demonstrated that the POC5^A429V mutation causes impaired cell-cycle progression, aberrant centrosomal protein interactions, and primary cilia retraction in osteoblasts.
> **Hypothesis**: The POC5 mutation directly drops $G_{mech}$ by physically truncating the sensory apparatus required to detect low-frequency gravitational drift.
