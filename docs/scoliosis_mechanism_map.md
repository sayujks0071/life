...
### 18. PIEZO1-Primary Cilia Axis
* **Path**: `PIEZO1` $\rightarrow$ `CILIA` $\rightarrow$ `Growth Plate Dysplasia`
* **Mechanism**: Compressive mechanical stress acts on the PIEZO1 channel, which forms an axis with primary cilia. High stress triggers ciliary disassembly and dysfunction in vertebral growth plate chondrocytes.
* **Role**: **Counter-Curvature Failure via Growth Plate Degeneration**. Normally, primary cilia and PIEZO1 act together in the mechanosensory loop to adjust growth in response to gravity. When compressive stress causes cilia degeneration, this loop is broken, leading to growth plate dysplasia and asymmetric ossification that ultimately compromise spinal stability.
* **Evidence**: Chen et al. (2025) demonstrated that the PIEZO1-primary cilia axis mediates compressive stress-induced growth plate degeneration and ossification in Adolescent Idiopathic Scoliosis.
