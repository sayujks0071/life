...
## Overview
The **Scoliosis Mechanism Map** traces the pathway **from molecules to curvature**, connecting molecular actors to tissue mechanics via **Biological Counter-Curvature**. The spine actively grows into an anticipatory S-shape to neutralize gravity. Scoliosis is not just a structural deformation, but a failure of this active molecular control system.

The organizing principle of **Biological Counter-Curvature** unifies these diverse molecular pathways, framing them all as components of a single active control system working to offset gravitational loading. Instead of viewing scoliosis as a collection of isolated defects (e.g., a "bone problem", a "muscle problem", or a "neural problem"), Counter-Curvature reveals how genetic variation, mechanical forces, and tissue metabolism feed into a unified mechanosensory loop. From foundational genes (like *HOX* and *PTCH1*) to structural tissues, this continuous feedback loop dynamically adjusts structural stiffness ($EI$) and biological target curvature ($\kappa_0$) via mechanotransductive gain ($G_{mech}$) to maintain spinal stability against gravity. When pathways like PIEZO1 signaling or ciliary Hedgehog patterning fail, the active control loop collapses, leading to Euler buckling and the emergent 3D deformity of scoliosis.
...
### 18. Ciliary Hedgehog Patterning
* **Path**: `PTCH1` $\rightarrow$ `Hedgehog Signaling` $\rightarrow$ `Rest Curvature (\kappa_0)`
* **Mechanism**: The Hedgehog signaling pathway, mediated by the primary receptor PTCH1 at the primary cilium, is crucial for vertebral segmentation and proper skeletal development.
* **Role**: **Baseline Geometry Setting**. Disruption in this pathway alters the skeletal baseline patterning and geometry, leading to abnormal structural alignments and asymmetric vertebral formation.
* **Evidence**: Hahn et al. (1996) established that PTCH1 mutations cause Gorlin syndrome (Nevoid Basal Cell Carcinoma), which prominently features rib anomalies and kyphoscoliosis (PMID: 8673105).
> **Hypothesis**: Impaired ciliary Hedgehog signaling introduces early, localized deviations in the target baseline geometry ($\kappa_0$), reducing the spine's initial tolerance against Euler buckling forces later during adolescent growth.
