...
### 18. BMSC Primary Cilia Defect
* **Path**: `CILIA` $\rightarrow$ `BMSC Defect` $\rightarrow$ `Osteogenesis Failure`
* **Mechanism**: Bone marrow stromal cells (BMSCs) normally rely on primary cilia to properly interpret mechanical forces into osteogenic differentiation signals.
* **Role**: **Impaired Bone Formation**. When primary cilia genes are defective in BMSCs, osteogenesis is disrupted. This compromises bone mineralization and tissue material quality ($EI$), making the spine more susceptible to buckling and leading to idiopathic scoliosis.
* **Evidence**: Zhang et al. (2025) demonstrated that primary cilia gene defects specifically within BMSCs can induce idiopathic scoliosis by disrupting their osteogenic potential.
> **Hypothesis**: BMSC cilia defects create a structural vulnerability where the spine fails to generate sufficient stiffness ($EI$) to match gravitational loading demands during adolescent growth spurts.
