...
### 17. Vestibulo-Sympathetic Bone Remodeling Axis
* **Path**: `Vestibular Input` $\rightarrow$ `Sympathetic Tone` $\rightarrow$ `B2AR` $\rightarrow$ `TMD`
* **Mechanism**: The vestibular system detects gravity and directly regulates bone mass via sympathetic nerves releasing norepinephrine onto osteoblast Beta-2 adrenergic receptors.
* **Role**: **Asymmetric Wedging**. Asymmetric vestibular function (e.g., otolith imbalance) drives asymmetric sympathetic outflow, leading to reduced bone formation (osteopenia) on the side with higher sympathetic tone, potentially leading to vertebral wedging characteristic of scoliosis progression.
* **Counter-Curvature Integration**: This neural-material bridge perfectly illustrates "Biological Counter-Curvature" as an active system: the spine's geometry serves as an informational readout of the organism's gravitational model. If the internal model of gravity is tilted (vestibular asymmetry), the sympathetic material controller remodels the spine to match this tilted reality, creating a physical counter-curvature to compensate for a sensory error.
* **Evidence**: Vignaux et al. (2013) demonstrated that vestibular signals regulate bone remodeling via the sympathetic nervous system.
