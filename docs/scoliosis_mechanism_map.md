...
### 18. Manganese-Dependent Glycosylation
* **Path**: `SLC39A8` $\rightarrow$ `Glycosylation` $\rightarrow$ `ECM Assembly`
* **Mechanism**: The SLC39A8 gene encodes ZIP8, a critical cellular importer of manganese. Intracellular manganese is an essential cofactor for glycosyltransferases (e.g., B4GALT1) responsible for protein glycosylation.
* **Role**: **Loss of Structural Stiffness**. Proper glycosylation is required for the correct assembly and cross-linking of extracellular matrix (ECM) components like collagen. A missense mutation in SLC39A8 impairs manganese influx, leading to hypoglycosylation, weakened ECM assembly, and reduced tissue stiffness ($EI$). This compromises the spine's passive mechanical resistance to buckling.
* **Evidence**: Haller et al. (2018) identified a missense variant (p.Ala391Thr) in SLC39A8 highly associated with severe adolescent idiopathic scoliosis (AIS), demonstrating reduced manganese transport and correlating with vertebral abnormalities.
> **Hypothesis**: Targeted dietary manganese supplementation in patients with SLC39A8 deficiency may rescue glycosylation pathways, restoring ECM stiffness and effectively raising the critical Euler buckling threshold to prevent progressive deformity.
