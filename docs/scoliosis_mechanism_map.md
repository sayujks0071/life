...
### 18. Glycine Neurotransmission Impairment
* **Path**: `GLY (Glycine Signaling)` $\rightarrow$ `Motor Control` $\rightarrow$ `Muscle Tone`
* **Mechanism**: Normal postural stability relies on balanced descending excitatory control and local inhibitory interneurons in the spinal cord. Glycine acts as a key inhibitory neurotransmitter.
* **Role**: **Loss of Inhibitory Balance**. Deficient glycine neurotransmission disrupts this delicate balance, leading to imbalanced paraspinal muscle activation. Without this refined motor control, the asymmetric muscle tone drives uneven loading across the spine.
* **Evidence**: Wang et al. (2024) demonstrated that impaired glycine neurotransmission directly causes adolescent idiopathic scoliosis, highlighting it as an underlying neuromuscular trigger.
> **Hypothesis**: Disruption of glycinergic inhibition introduces a persistent asymmetric bias in the muscle tension network, undermining the active Counter-Curvature control system's ability to symmetrically counteract gravitational loads.
