...
### 18. Proprioceptive Delay (\tau)
* **Path**: `ADGRG6 / MBP` $\rightarrow$ `Myelination` $\rightarrow$ `Proprioceptive Delay (\tau)` $\rightarrow$ `Gain (G_{mech})`
* **Mechanism**: Neural conduction velocity relies on proper myelination governed by genes like ADGRG6 (GPR126) and MBP.
* **Role**: **Terminal Derivative Gain Gap**. Counter-curvature requires real-time proprioceptive feedback to calculate the derivative term ($K_d$) of postural sway. If myelination is defective, the communication delay ($\tau$) increases. A large phase lag in this active control loop effectively creates a "Derivative Gain Gap", neutralizing damping and causing scoliotic buckling under gravity.
* **Evidence**: Karner et al. (2015) demonstrated that Gpr126/Adgrg6 deletion causes idiopathic scoliosis-like deformity linked to delayed or defective neural tissue organization.
> **Hypothesis**: The onset of adolescent scoliosis coincides with a threshold breach where proprioceptive delay ($\tau$) exceeds the biomechanical critical damping time of the rapidly elongating spine.
