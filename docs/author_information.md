# Author Information

## Corresponding Author

**Dr. Sayuj Krishnan S, MBBS, DNB (Neurosurgery)**  
Consultant Neurosurgeon and Spine Surgeon  
Yashoda Hospitals, Malakpet  
Hyderabad, India  

**Email:** dr.sayujkrishnan@gmail.com

---

## Article Type

**Original Research Article**

---

## Usage

This information has been added to:
- ✅ Cover letter template (`docs/cover_letter_expansion_template.md`)
- ✅ Citation file (`CITATION.cff`)
- ⏳ Manuscript LaTeX (add when finalizing author block)

---

## Notes

- **MBBS**: Bachelor of Medicine, Bachelor of Surgery
- **DNB**: Diplomate of National Board (Neurosurgery)
- **Affiliation**: Yashoda Hospitals, Malakpet, Hyderabad, India

---

## For Manuscript

When finalizing the LaTeX manuscript, add this to the author block:

```latex
\author{Dr. Sayuj Krishnan S, MBBS, DNB (Neurosurgery)}
\affiliation{Consultant Neurosurgeon and Spine Surgeon, Yashoda Hospitals, Malakpet, Hyderabad, India}
\email{dr.sayujkrishnan@gmail.com}
```

Or use the journal's specific author format if required.

