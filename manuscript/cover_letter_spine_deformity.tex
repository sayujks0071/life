...
We submit our manuscript entitled \textbf{``Active Geometric Maintenance of the Spinal S-Curve Against Gravity: An Information--Mechanical Coupling Model for Adolescent Idiopathic Scoliosis Onset''} for consideration as an Original Article in \textit{Spine Deformity}. There is no journal-imposed deadline for a standard original article as of 22 August 2026. If the editors prefer, the manuscript may also be considered for the journal's AI / machine-learning collection (Springer currently lists that collection Status: Open, deadline Ongoing; the original 20 May 2026 23:59 CST cutoff is past). If that collection is selected, please do not use guest editors as reviewers.
...
All three predictions are testable in existing AIS cohorts without new instrumentation. Source code, data, and the full reproducibility package are publicly available at \url{https://github.com/sayujks0071/life}. This revision adds an open SpineWeb/AASCE landmark-geometry check (609 AP cases; 534 usable). That check is 2D geometry consistency, not progression validation: the public package has no Risser/Sanders stage, serial Cobb, or treatment labels. Patient-level clinical records are not analysed in this submission.

This manuscript has not been submitted elsewhere. We declare no competing interests; no external funding was received.
...
\item \textbf{Prof.\ Carl-Eric Aubin} (Polytechnique Montr\'{e}al) --- spine biomechanics, Cosserat-rod modeling. \emph{Omit this suggestion if submitting to the AI / machine-learning collection, where he is a guest editor.}
