...
\subsection{Deterministic IEC Beam Model and Multi-Segment Extension}
...
To model the emergence of distinct Lenke curve types, we extend the deterministic model to a multi-segment Cosserat rod. The spatial patterning of the curves is formulated as a generalized eigenvalue problem:
\begin{equation}
(B(s) y'')'' = \lambda Q(s) y
\label{eq:generalized_eigenvalue}
\end{equation}
where $B(s)$ is the regional bending stiffness (incorporating rib cage buttressing and local variations) and $Q(s)$ is the regional instability drive, proportional to the ratio of local proprioceptive delay $\tau(s)$ to local damping $b(s)$. This formulation replaces arbitrary sine wave assumptions, allowing the specific Lenke curve morphologies to emerge naturally as the fundamental buckling eigenmodes of the heterogeneous spinal system.
