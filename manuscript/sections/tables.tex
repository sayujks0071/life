...
\begin{table}[h!]
\centering
\caption{Thermodynamic cost proteins mapped to the free energy dissipation functional (Eq.~\ref{eq:dissipation}). Proteins are grouped by their primary dissipation term ($\eta_p$ for proprioceptive feedback, $\eta_a$ for active maintenance, and $\Gamma_m$ for metabolic supply). AlphaFold structural metrics (Anisotropy, Morphology, pLDDT) highlight the Demand-Supply Anisotropy Gap.}
\label{tab:thermodynamic_cost_proteins}
\small
\begin{tabular}{llclclclcl}
\toprule
...
\midrule
\multicolumn{8}{l}{\textit{Active Maintenance ($\eta_a$)}} \\
...
\midrule
\multicolumn{8}{l}{\textit{Metabolic Supply ($\Gamma_m$)}} \\
