...
Complementing this geometric analysis, we computed the thermodynamic cost of countercurvature, defined as $P_{\mathrm{counter}} \propto L^2 \langle |\kappa_{\mathrm{IEC}} - \kappa_{\mathrm{passive}}|^2 \rangle$. Under isometric growth conditions (where cross-sectional area $A \propto L^2$), our simulations reveal that the metabolic cost scales quadratically ($P_{\mathrm{counter}} \sim L^2$), reflecting the increasing active moment required to counteract the gravitational moment $M_g \sim L^4$. When compared to a proprioceptive supply capacity that scales sublinearly ($S_{\mathrm{proprio}} \sim L^{0.5}$), we identify an ``Energy Deficit Window'' where metabolic demand exceeds supply for lengths $L > L_{\mathrm{crit}} \approx 0.35$ m (Fig.~\ref{fig:energy_deficit_window}). % Results verified by scripts/experiments/experiment_energy_deficit_window.py
At $L=0.45$ m, the metabolic demand exceeds supply by 41.3\% ($P_{\mathrm{counter}} \approx 1.41 S_{\mathrm{proprio}}$).
This finding aligns with the clinical onset of Adolescent Idiopathic Scoliosis (AIS) during the pubertal growth spurt, suggesting that rapid axial elongation outpaces the maturation of the proprioceptive-metabolic axis, leaving the spine energetically insolvent and vulnerable to asymmetric buckling.
