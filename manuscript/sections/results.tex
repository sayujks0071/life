...
\subsection{Lenke Curve Type Diversity: Spatial Heterogeneity of the Deficit}

Our framework makes a central prediction absent from all prior AIS theories: the spatial localization of the Energy Deficit Window along the spine determines the specific curve pattern, known clinically as the Lenke curve type. While the single-link inverted pendulum model captures the global onset of instability, extending the model to a multi-segment Cosserat rod accounts for regional variations in parameters: regional stiffness $EI(s)$ (e.g., rib cage buttressing), segmental differences in proprioceptive delay $\tau(s)$ (mechanoreceptor density), local damping $b(s)$ (paraspinal muscle bulk, disc composition), and asymmetric loading (e.g., aortic pulsation).

These regional differences determine which vertebral segments destabilize first once the global threshold is breached. Solving the generalized eigenvalue problem $(B(s) y'')'' = \lambda Q(s) y$ reveals that the resulting curve morphology emerges as the dominant eigenmode of the multi-segment system, naturally mapping to all six Lenke types:
\begin{itemize}
    \item \textbf{Type 1 (Main Thoracic):} Emerges when the deficit localizes where the thoracic spine carries a large moment arm over the thinnest paraspinal muscle mass, with minimal rib cage stiffening.
    \item \textbf{Type 2 (Double Thoracic):} Arises from simultaneous, localized deficits in both the proximal and main thoracic regions, often corresponding to steep transition zones in the patterning field.
    \item \textbf{Types 3 \& 4 (Double and Triple Major):} Represent cascading bifurcations where compensatory curves (lumbar or cervical) themselves experience sufficient loading to cross the instability threshold, generating a multi-nodal eigenmode.
    \item \textbf{Type 5 (Thoracolumbar/Lumbar):} The lumbar spine has greater inherent torsional resistance. A primary lumbar deficit shifts the local $L_{crit}$ rightward, reflecting later onset during the growth spurt.
    \item \textbf{Type 6 (TL/L Main with Minor Thoracic):} A mixed presentation where the lumbar zone is the primary site of collapse, with a minor compensatory or secondary thoracic mode.
\end{itemize}
This mapping confirms that our thermodynamic framework provides not only the upstream trigger for idiopathic scoliosis (global instability onset) but also the downstream patterning mechanism (multi-segment buckling eigenmodes).
