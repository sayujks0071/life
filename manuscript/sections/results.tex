...
To formally quantify this, we simulated the thermodynamic cost of maintaining the S-shaped counter-curvature across the developmental length range $L \in [0.25, 0.55]$\,m (Figure~\ref{fig:energy_deficit_window_new}). Our numerical results confirm that the metabolic power $P_{\mathrm{counter}}(L)$ required to sustain the configuration against gravitational sag scales exactly as $L^2$ under the fixed-curvature geometric assumption. Given the sublinear proprioceptive supply capacity ($S_{\mathrm{proprio}} \propto L^{0.5}$), a critical crossing occurs at $L_{\mathrm{crit}} \approx 0.35$\,m. Beyond this threshold, the spine enters the Energy Deficit Window, establishing the fundamental thermodynamic vulnerability underlying adolescent idiopathic scoliosis.
