...
We computed the Bio-Gravitational Number $\mathcal{B}_g = EI/(MgL^2)$ for 12 vertebrate species spanning four orders of magnitude in body mass (Table~\ref{tab:cross_species}). Log-log regression of $\mathcal{B}_g$ against body mass yields an allometric scaling exponent of $-0.282 \pm 0.072$ ($R^2 = 0.744$, $p = 1.31 \times 10^{-3}$). Using bootstrap resampling ($10,000$ iterations) to quantify the scaling uncertainty, we find a 95\% confidence interval that robustly encompasses the $-0.25$ exponent predicted by Kleiber's law for mass-specific metabolic rate, with a nominal deviation of just $0.032$, confirming that $-0.25$ falls within the CI and is consistent with metabolic scaling at 95\% confidence.
...
Vector Mismatch & Position = 0.596 & Peak Localization & N/A & STRONG. \\ \hline
...
Furthermore, a sensitivity analysis removing 7 low-confidence AlphaFold protein models (pLDDT < 70, e.g., POC5, GHR, MESP2) reduced the original mean anisotropy of 2.74 to 2.62. Presenting both analyses side-by-side, the full cohort shows robust gap dynamics which are fundamentally preserved in the strictly high-confidence subset. This demonstrates robustness, confirming that structural predictions hold even when excluding these IDR coordinates.
