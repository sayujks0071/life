...
Our new simulation results precisely compute the thermodynamic cost of countercurvature across the developmental window. The metabolic power required to sustain the counter-curvature $P_{\mathrm{counter}}(L)$ is shown to scale strongly as $L^2$. By projecting a sublinear ($L^{0.5}$ to $L^{1.0}$) maturation of proprioceptive supply capacity $S_{\mathrm{proprio}}$, we pinpoint the critical length $L_{\mathrm{crit}} \approx 0.35$\,m where supply curves intersect with metabolic demand. For $L > L_{\mathrm{crit}}$, the system enters the Energy Deficit Window, establishing a robust physical trigger for scoliotic buckling (Fig.~\ref{fig:energy_deficit_window_new}).
