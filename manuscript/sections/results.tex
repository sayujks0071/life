...
Crucially, the thermodynamic cost of maintaining the S-shaped counter-curvature against gravity, $P_{\mathrm{counter}}(L)$, scales as $L^2$ under the fixed-curvature assumption, while the proprioceptive supply capacity scales sublinearly ($L^{0.5}$ or $L^{1.0}$). Our model explicitly identifies the critical length $L_{\mathrm{crit}} \approx 0.35$\,m where this Energy Deficit Window opens (Fig.~\ref{fig:energy_deficit_window}), leaving the system under-supplied and mechanically vulnerable to the asymmetric buckling phenomena described above.
