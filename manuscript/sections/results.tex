...
Table~\ref{tab:comprehensive_stats} provides a detailed overview of these findings.
...
Furthermore, a sensitivity analysis removing 7 low-confidence AlphaFold protein models ($\text{pLDDT} < 70$, specifically including POC5, GHR, and MESP2) reduced the original mean anisotropy of 2.74 to 2.62. These low scores predominantly reflect Intrinsically Disordered Regions (IDRs) common in metabolic supply signaling hubs. This demonstrates robustness, confirming that structural predictions hold even when strictly relying on high-confidence structural coordinates.
