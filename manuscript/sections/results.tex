...
To validate this energetic mismatch, we performed dynamic simulations scaling the spinal length $L \in [0.25, 0.55]$~m. As shown in Figure~\ref{fig:energy_deficit_window}, the metabolic power $P_{\mathrm{counter}}(L)$ required to maintain counter-curvature against gravity scales strictly as $L^2$. In contrast, proprioceptive supply capacity matures sublinearly ($L^{0.5}$), leading to a critical crossing at $L_{\mathrm{crit}} \approx 0.35$~m. Beyond this threshold, the Energy Deficit Window opens, explaining the precise timing of biomechanical vulnerability observed during the adolescent growth spurt.
