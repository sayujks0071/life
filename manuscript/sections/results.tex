...
Specifically, we quantified the thermodynamic cost of countercurvature $P_{\mathrm{counter}}(L)$ across the developmental length range $L \in [0.25, 0.55]$\,m. The metabolic power required to maintain the S-curve against passive gravitational sag scales precisely as $L^2$. In contrast, the proprioceptive supply capacity $S_{\mathrm{proprio}}$ follows a sublinear maturation trajectory, leading to an intersection at $L_{\mathrm{crit}} \approx 0.35$\,m (Figure~\ref{fig:energy_deficit_window}). For $L > L_{\mathrm{crit}}$, the system enters an ``Energy Deficit Window'' where the cost of maintaining geometric fidelity exceeds the available metabolic flux, rendering the adolescent spine structurally vulnerable to deformation and providing a mechanism for metabolic buckling.
