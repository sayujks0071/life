...
Using the IEC framework, we further characterized the thermodynamic cost of maintaining the counter-curvature during this critical period. We determined that the metabolic power $P_{\mathrm{counter}}(L)$ scales proportionally with $L^2$. In contrast, the proprioceptive supply capacity $S_{\mathrm{proprio}}$ follows a sublinear trajectory $L^{0.5}$ (Fig.~\ref{fig:energy_deficit_window}). This mismatch opens an ``Energy Deficit Window'' that specifically emerges when the spinal length exceeds a critical threshold $L_{\mathrm{crit}} \approx 0.35$\,m. At a simulated typical adolescent length of $0.45$\,m, this mechanism alone accounts for an approximate $41\%$ energy deficit, structurally mapping the adolescent growth spurt to peak vulnerability for scoliotic buckling.
