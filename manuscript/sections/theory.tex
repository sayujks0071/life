...
We introduce a scalar information field $I(s, t) \in [0, 1]$ representing the coarse-grained expression level of anterior-posterior patterning morphogens (e.g., HOX genes, SHH/BMP gradients). The spatial gradient of this field, $\nabla I$, acts as a "curvature generator," driving the resting geometry of the rod away from the straight configuration. We define three key coupling constants that translate this information into mechanical action:

\paragraph{Geometric Coupling ($\chi_\kappa$).}
This dimensionless constant links the information gradient to the intrinsic rest curvature $\boldsymbol{\kappa}_{\mathrm{rest}}$ (the target shape):
\begin{equation}
\boldsymbol{\kappa}_{\mathrm{rest}}(s) = \boldsymbol{\kappa}_{\mathrm{gen}} + \chi_\kappa \nabla I(s),
\label{eq:kappa_rest}
\end{equation}
where $\boldsymbol{\kappa}_{\mathrm{gen}}$ is the generic curvature. A high $\chi_\kappa$ implies that sharp transitions in morphogen identity (e.g., cervicothoracic junction) correspond to high intrinsic curvature targets.

\paragraph{Morphomechanical Coupling ($\chi_M$).}
This constant (dimensions $[ML^3T^{-2}]$) represents the "stiffness of the information field." It relates the information gradient to the active biological moment $\mathbf{M}_{bio}$ generated to oppose gravity:
\begin{equation}
\mathbf{M}_{bio} = \chi_M (\mathbf{\Lambda} \cdot \nabla I),
\label{eq:active_moment}
\end{equation}
where $\mathbf{\Lambda}$ is the tissue anisotropy tensor. This formalizes the idea that the organism must actively generate torque to maintain the S-shape.

\paragraph{The Bio-Gravitational Number ($\mathcal{B}_g$).}
To quantify stability, we introduce the dimensionless Bio-Gravitational Number, comparing the active biological moment to the passive gravitational moment:
\begin{equation}
\mathcal{B}_g = \frac{\chi_M \langle |\nabla I| \rangle}{\rho A g L^2}.
\label{eq:bg_number}
\end{equation}
When $\mathcal{B}_g > 1$, the organism dominates gravity (stable counter-curvature). When $\mathcal{B}_g < 1$, gravity dominates, leading to buckling (scoliosis).

Following the bimodal Gaussian parameterization already defined in Methods (Eq.~\ref{eq:info_field_spinal}), we model $I(s)$ with peaks corresponding to the cervical ($A_c=0.5, s_c=0.80$) and lumbar ($A_l=0.7, s_l=0.25$) regions of high vertebral identity specification.
