...
The spine centerline is modeled as a continuous curve $\mathbf{r}(s,t) \in \mathbb{R}^3$, parameterized by arc length $s \in [0, L]$. At every point $s$, we attach a local orthonormal material frame $\{\mathbf{d}_1(s,t), \mathbf{d}_2(s,t), \mathbf{d}_3(s,t)\}$ to describe the orientation of the cross-section~\cite{goriely2017mathematics, gazzola2018forward}. Here, $\mathbf{d}_3(s,t)$ is the director aligned with the tangent (assuming no shear), while $\mathbf{d}_1(s,t)$ and $\mathbf{d}_2(s,t)$ span the vertebral cross-section. The kinematic evolution is governed by:
...
We introduce a scalar information field $I(s) \in [0, 1]$ to capture the coarse-grained expression of anterior-posterior patterning morphogens, specifically HOX genes. Consistent with the bimodal Gaussian parameterization (Methods Eq.~\ref{eq:info_field_spinal}), $I(s)$ is modeled as a distribution with peaks at the cervical ($s_c=0.80, A_c=0.5$) and lumbar ($s_l=0.25, A_l=0.7$) enlargements. These peaks delineate regions of high vertebral identity specification. The gradient of this field, $\nabla I$, serves as the "curvature generator" driving the rest geometry through the coupling constant $\chi_\kappa$:
\begin{equation}
...
When $\mathcal{B}_g > 1$, the organism dominates gravity. This framework is physically grounded in the structural specificity of HOX proteins (e.g., HOXC8, HOXB13). AlphaFold structural analysis confirms that these factors possess rigid DNA-binding domains, which are capable of enforcing sharp identity boundaries.
...
The \emph{proprioceptive feedback term} $\eta_p$ is borne by three proteins: the vector mechanosensor PIEZO2 (anisotropy~4.44, fibrous/extended morphology), the scalar sensor PIEZO1 (anisotropy~3.90, 2521~residues), and the spindle-maintenance transcription factor EGR3 (anisotropy~3.76, 64\% intrinsically disordered). Their high anisotropy reflects the extended, orientation-dependent structures required for directional sensing---structures that are intrinsically expensive to maintain in proper alignment.

The \emph{active moment term} $\eta_a$ is borne by the molecular machines that generate tonic contraction: the dystrophin-glycoprotein complex linker Dystrophin (DMD; Score 100), the myosin light chain kinase (MYLK; Score 100), the nuclear stiffness regulator Lamin~A/C (LMNA; anisotropy~4.75), the mechanosensitive crosslinker Filamin~A (FLNA), and the AIS-associated transcription factor LBX1~\cite{kou2013lbx1}. Their high structural complexity reflects the extreme metabolic cost of the cytoskeletal-nuclear axis that transmits gravitational loads.

The \emph{basal maintenance term} $\Gamma_m$ includes the critical metabolic and growth regulators that must scale with spinal length: the mitochondrial master regulator PPARGC1A (PGC-1$\alpha$; Score 100), the growth hormone receptor GHR, the insulin-like growth factor receptor IGF1R, and the circadian clock component ARNTL (BMAL1), alongside the structural matrix protein COL1A1 and the chondrogenic factor SOX9. These proteins represent the "supply side" of the energy equation, whose upregulation is required to meet the increasing metabolic demand of the growing spine.

This metabolic asymmetry provides the molecular basis for the energy deficit window: the structural demand proteins (DMD, FLNA, LMNA) must physically support the growing tissue mass against gravity, while the supply-side regulators (PPARGC1A, GHR) must drastically ramp up metabolic flux to meet this nonlinear cost.
