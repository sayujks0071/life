...
The Human spine is modeled as a one-dimensional Cosserat rod embedded in three-dimensional Euclidean space. However, in the IEC framework, we treat the rod's reference configuration not as a flat segment, but as a manifold whose intrinsic geometry is warped by a developmental information field $I(s)$. We draw a formal analogy to General Relativity (GR), where the geometry of space $g_{\mu\nu}$ is determined by the mass-energy distribution $T_{\mu\nu}$. This conceptual bridge suggests that information may play a geometric role in physical systems.
...
The \emph{proprioceptive feedback term} $\eta_p$ is borne by four proteins: the vector mechanosensor PIEZO2 (anisotropy~4.44, fibrous/extended morphology), the scalar sensor PIEZO1 (anisotropy~3.90, 2521~residues), the spindle-maintenance transcription factor EGR3 (anisotropy~3.76, 64\% intrinsically disordered), and the proprioceptive neuron development factor RUNX3 (anisotropy~2.06). Their high anisotropy reflects the extended, orientation-dependent structures required for directional sensing---structures that are intrinsically expensive to maintain in proper alignment.

The \emph{active moment term} $\eta_a$ is borne by the molecular machines that generate tonic contraction: the dystrophin-glycoprotein complex linker Dystrophin (DMD; Score 100), the myosin light chain kinase (MYLK; Score 100), the nuclear stiffness regulator Lamin~A/C (LMNA; anisotropy~4.75), the mechanosensitive crosslinker Filamin~A (FLNA), the intermediate filament Vimentin (VIM; anisotropy~7.5), and the AIS-associated transcription factor LBX1~\cite{kou2013lbx1}. Their high structural complexity reflects the extreme metabolic cost of the cytoskeletal-nuclear axis that transmits gravitational loads.
...
\end{enumerate}
% Verified: Sections and Table present per instructions.
