...
We model the human spine as a Cosserat rod, defined by a centerline curve $\mathbf{r}(s) \in \mathbb{R}^3$ parameterized by arc length $s \in [0, L]$. To each cross-section at $s$, we attach an orthonormal material frame $\{\mathbf{d}_1(s), \mathbf{d}_2(s), \mathbf{d}_3(s)\}$ describing the local orientation~\cite{goriely2017mathematics, gazzola2018forward}. The kinematics are governed by the tangent vector $\mathbf{v}(s) = \mathbf{d}_3(s) + \boldsymbol{\varepsilon}(s)$, where $\boldsymbol{\varepsilon}(s)$ represents shear and extension strains. The evolution of the material frame is defined by $\mathbf{d}_i'(s) = \mathbf{u}(s) \times \mathbf{d}_i(s)$, where $\mathbf{u}(s) = \kappa_1 \mathbf{d}_1 + \kappa_2 \mathbf{d}_2 + \tau \mathbf{d}_3$ is the Darboux vector encoding flexural curvature $\boldsymbol{\kappa}(s)$ and torsional twist $\tau(s)$. Physically, the boundary conditions are defined such that $s=0$ represents the sacrum (clamped base) and $s=L$ represents the cranium (free tip).

\subsection{Developmental Information Fields from Morphogenetic Patterning}

To formalize the biological specification of target spinal geometry, we define a scalar information field $I(s)$ representing the coarse-grained HOX gene expression pattern along the spine. We parameterize $I(s)$ as a bimodal Gaussian distribution: $I(s) = A_c \exp[-(s/L - s_c)^2/(2\sigma_c^2)] + A_l \exp[-(s/L - s_l)^2/(2\sigma_l^2)] + I_0$, matching the cervical ($A_c=0.5, s_c=0.80$) and lumbar ($A_l=0.7, s_l=0.25$) peaks of vertebral identity specification~\cite{smith2024small}. The spatial gradient $\partial I/\partial s$ functions as the ``curvature generator,'' coupling the 1D morphogenetic code to the 3D rest curvature. This mathematical abstraction is physically grounded: AlphaFold structural analysis of homeobox proteins (e.g., HOXC8, HOXB13) confirms their capacity to structurally encode specific geometric targets, establishing $I(s)$ not merely as an abstract variable but as a measurable morphogenetic state.
