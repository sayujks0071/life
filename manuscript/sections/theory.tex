...
We model the spine as a Cosserat rod defined by a centerline curve $\mathbf{r}(s) \in \mathbb{R}^3$ parameterized by arc length $s \in [0, L]$. To each point $s$, we attach an orthonormal material frame $\{\mathbf{d}_1(s), \mathbf{d}_2(s), \mathbf{d}_3(s)\}$ describing the orientation of the cross-section~\cite{goriely2017mathematics}. The kinematics are characterized by the curvature vector $\boldsymbol{\kappa}(s)$ and twist $\tau(s)$ derived from the Darboux vector $\mathbf{u}(s) = \kappa_1 \mathbf{d}_1 + \kappa_2 \mathbf{d}_2 + \tau \mathbf{d}_3$, and the strain vector $\boldsymbol{\varepsilon}(s)$ capturing extensibility and shear~\cite{gazzola2018forward}. Physically, we identify $s=0$ with the sacrum (clamped boundary condition) and $s=L$ with the cranium (free boundary condition), representing the aggregate mechanical axis of the vertebral column.

\subsection{Developmental Information Fields from Morphogenetic Patterning}

We introduce a scalar information field $I(s) \in [0, 1]$ representing the coarse-grained expression level of anterior-posterior patterning morphogens (e.g., HOX genes). Following the parameterization in Eq.~\ref{eq:info_field}, $I(s)$ is modeled as a bimodal Gaussian distribution with peaks corresponding to the cervical ($A_c=0.5, s_c=0.80$) and lumbar ($A_l=0.7, s_l=0.25$) regions of high vertebral identity specification. The spatial gradient $\partial I/\partial s$ acts as a "curvature generator," driving the resting geometry of the rod away from the straight configuration. This posits that morphogenetic information encodes a target metric for the developing spine. AlphaFold analysis of key HOX proteins (e.g., HOXC8, HOXB13) reveals distinct structural rigidities that may underpin this spatial specificity, as detailed below.
