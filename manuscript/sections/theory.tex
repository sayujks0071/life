...
The \emph{active moment term} $\eta_a$ is borne by the molecular machines that generate tonic contraction: the intermediate filament Vimentin (VIM; anisotropy~7.47, the highest in the dataset), Lamin~A/C (LMNA; anisotropy~4.75), Caveolin-1 (CAV1), Filamin~A (FLNA), and the AIS-associated transcription factor LBX1~\cite{kou2013lbx1}, alongside the contractile regulators Myosin Light Chain Kinase (MYLK) and Dystrophin (DMD). Their mean anisotropy reflects the extreme structural cost of the cytoskeletal-nuclear axis that transmits gravitational loads.

The \emph{basal maintenance term} $\Gamma_m$ is borne by structurally more compact proteins involved in metabolic supply and growth signaling: Type~I collagen (COL1A1), the growth plate driver SOX9, the Growth Hormone Receptor (GHR), the circadian clock regulator BMAL1 (ARNTL), the mitochondrial master regulator PPARGC1A, and the IGF-1 Receptor (IGF1R). Their lower mean anisotropy compared to the demand-side proteins confirms that the energy supply apparatus is structurally cheaper than the mechanotransduction apparatus it must power.
