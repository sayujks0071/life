...
Following the bimodal Gaussian parameterization already defined in Methods (Eq.~\ref{eq:info_field_spinal}), we model $I(s)$ with peaks corresponding to the cervical ($A_c=0.5, s_c=0.80$) and lumbar ($A_l=0.7, s_l=0.25$) regions of high vertebral identity specification. Physically, these peaks represent zones where the specification of vertebral identity is most robust, driven by the combinatorial expression of HOX genes. The spatial gradient of this field, $\partial I/\partial s$, acts as the "curvature generator" that drives the intrinsic rest curvature away from the straight configuration. AlphaFold analysis of key HOX proteins (e.g., HOXC8, HOXB13) confirms distinct structural rigidities that may underpin this spatial specificity~\cite{jumper2021alphafold}.
