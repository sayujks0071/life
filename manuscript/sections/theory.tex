...
The human vertebral column is structurally modeled as an active, continuous Cosserat rod embedded in $\mathbb{R}^3$, an idealization rigorously justified by its high aspect ratio ($L/d \approx 13$) and the dense, segmented distribution of intervertebral joints which act as quasi-continuous elastic hinges~\cite{goriely2017mathematics, gazzola2018forward}. Let the centerline curve be denoted by $\mathbf{r}(s) \in \mathbb{R}^3$, parameterized by arc length $s \in [0, L]$. To fully capture the macroscopic 3D kinematics, including flexural bending, shear, and torsional deformations, an orthonormal material frame $\{\mathbf{d}_1(s), \mathbf{d}_2(s), \mathbf{d}_3(s)\}$ is attached to each cross-section.

The spatial evolution of the rod's configuration is thus strictly governed by the kinematic equations:
\begin{align}
\partial_s \mathbf{r}(s) &= \mathbf{d}_3(s) + \boldsymbol{\varepsilon}(s), \\
\partial_s \mathbf{d}_i(s) &= \mathbf{u}(s) \times \mathbf{d}_i(s),
\label{eq:cosserat_kinematics}
\end{align}
where $\boldsymbol{\varepsilon}(s)$ captures the shear and axial extension strains for extensibility. The Darboux vector $\mathbf{u}(s) = \kappa_1 \mathbf{d}_1 + \kappa_2 \mathbf{d}_2 + \tau \mathbf{d}_3$ encodes the bending curvature vector $\boldsymbol{\kappa}(s)$ and the torsional twist $\tau(s)$. Physically, we impose a clamped boundary condition at the sacrum ($s=0$), reflecting pelvic anchoring, and a free-end condition at the cranium ($s=L$).

\subsection{Developmental Information Fields from Morphogenetic Patterning}

Rather than treating the spine as a passive beam reacting to gravitational loading, our framework strictly formalizes developmental inputs as a macroscopic scalar information field $I(s)$. This field acts as a coarse-grained representation of the anterior-posterior HOX gene expression pattern along the spine.

Empirically, we parameterize $I(s)$ via a bimodal Gaussian distribution, with distinct maxima corresponding to regions of high vertebral identity specification: a cervical peak ($A_c=0.5, s_c=0.80$) and a lumbar peak ($A_l=0.7, s_l=0.25$).

The gradient of this morphogenetic field, $\partial I / \partial s$, acts as an active ``curvature generator.'' It induces a non-zero rest curvature that drives the target intrinsic geometry:
\begin{equation}
\boldsymbol{\kappa}_{\mathrm{rest}}(s) = \boldsymbol{\kappa}_{\mathrm{gen}} + \chi_\kappa \frac{\partial I(s)}{\partial s}.
\label{eq:kappa_rest}
\end{equation}
At the molecular level, this structural specificity is confirmed by AlphaFold analysis of critical HOX proteins (e.g., HOXC8, HOXB13).
...
\begin{itemize}
    \item \textbf{Demand-side mechanosensory proteins} (e.g., Vimentin, Lamin~A/C, PIEZO2): Characterized by high structural anisotropy (mean $3.08 \pm 1.44$ from AlphaFold v6 analysis, Table~\ref{tab:thermodynamic_cost_proteins_dissipation}), extended conformations, and high metabolic maintenance costs due to continuous conformational cycling.
