...
\begin{figure}[h!]
    ...
    \caption{\textbf{The Allometric Trap.} (A)~The fundamental problem: a passive beam inevitably sags into a C-shape under gravity (blue), while the living human spine acts as a \textbf{Thermodynamic Standing Wave}, actively maintaining an S-shaped counter-curvature (orange) at a continuous metabolic cost $P_{\mathrm{counter}}$. This counter-curvature functions as an active control engine minimizing the metric tensor deviation $\widehat{D}_{\mathrm{geo}}$ between the HOX-encoded developmental information field and the structural reality. (B)~The Bio-Gravitational Number $\mathcal{B}_g = EI/(MgL^2)$ across vertebrate species. Species above the threshold ($\mathcal{B}_g > 0.1$, dashed line) are passively stable. Humans ($\mathcal{B}_g \approx 0.01$) sit deep within the Allometric Trap, relying exclusively on high-frequency mechanosensory feedback (via PIEZO1/2 and proprioceptive networks) and active metabolic supply to prevent geometric collapse. (C)~The Energy Deficit Window: the metabolic cost of maintaining the counter-curvature control loop (red, scaling as $L^4$) outpaces the nutrient supply capacity (blue, scaling as $L^2$) beyond a critical developmental length $L_{\mathrm{crit}} \approx 0.35$\,m. At $L = 0.45$\,m (the peak of the adolescent growth spurt), the metabolic deficit reaches ${\sim}41\%$. This structural energy crisis artificially increases proprioceptive delay, rendering the spine vulnerable to micro-mechanical buckling. We propose this physical sequence manifests clinically as Adolescent Idiopathic Scoliosis---driven by the ``Derivative Gain Trap'' where delayed sensory feedback from metabolic constraints becomes intrinsically destabilizing.}
