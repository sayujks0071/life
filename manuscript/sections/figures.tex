...
\begin{figure}[h!]
    ...
    \caption{\textbf{Scoliosis as Metabolic Buckling.} (A)~Lateral scoliosis index $S_{\mathrm{lat}}$ versus coupling strength $\chi_\kappa$ for varying asymmetry levels. (B)~Bifurcation diagram showing amplification of small patterning errors ($\varepsilon_{\mathrm{asym}} \sim 5\%$) into macroscopic deformities (Cobb $> 15^\circ$) in the information-dominated regime. This boundary represents the \emph{Derivative Gain Trap}, where increased neuromotor delay $\tau$ due to the Energy Deficit Window crosses a critical threshold $\tau_c$, converting stable active control into scoliotic buckling~\cite{milton2009time, landry2005dynamics}. (C)~Scoliosis regime map in $(\chi_\kappa, \varepsilon)$ space. (D)~Amplification factor ($S_{\mathrm{lat}}/\varepsilon$): in the high-$\chi_\kappa$ regime, small asymmetries are amplified over 100-fold via the Vector Mismatch mechanism.}
