...
\begin{figure}[h!]
    ...
    \caption{\textbf{The Allometric Trap.} (A)~The fundamental problem: a passive beam sags into a C-shape under gravity (blue), while the living spine acts as a \textbf{Thermodynamic Standing Wave}, actively maintaining an S-shaped counter-curvature (orange) at continuous metabolic cost $P_{\mathrm{counter}}$. (B)~The Bio-Gravitational Number $\mathcal{B}_g = EI/(MgL^2)$ across vertebrate species. Species above the threshold ($\mathcal{B}_g > 0.1$, dashed line) are passively stable; humans ($\mathcal{B}_g \approx 0.01$) are deep within the Allometric Trap, relying on constant mechanosensory feedback and active metabolic maintenance to prevent geometric collapse. (C)~The Energy Deficit Window: the metabolic cost of counter-curvature (red, scaling as $L^2$) outpaces the proprioceptive supply capacity (blue, scaling as $L^{0.5}$) beyond a critical length $L_{\mathrm{crit}} \approx 0.35$\,m. At $L = 0.45$\,m (adolescent spine), the deficit reaches ${\sim}41\%$, rendering the structure vulnerable to metabolic buckling.}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{../outputs/figures/energy_deficit_window.png}
    \caption{\textbf{The Energy Deficit Window.} Metabolic power $P_{\mathrm{counter}}$ (red) required to maintain counter-curvature scales strongly as $L^2$, while proprioceptive supply capacity (blue, dashed) scales sublinearly ($L^{0.5}$ and $L^{1.0}$). Their intersection at $L_{\mathrm{crit}} \approx 0.35$\,m marks the precise onset of the Energy Deficit Window. For $L > 0.35$\,m, such as during the adolescent growth spurt, the metabolic demand outpaces supply, severely compromising the structural stability of the spine and precipitating scoliotic buckling.}
    \label{fig:energy_deficit_window_new}
\end{figure}
