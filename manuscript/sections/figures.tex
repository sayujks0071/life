...
\begin{figure}[h!]
    ...
    (D) Microgravity adaptation ($g \to 0$) and \textbf{Spinal Jetlag}. The system relaxes towards the information reference state. The reduced mechanical strain $\gEff$ shown here corresponds to the \textbf{Convective Shutdown}~\cite{sato2025convective}, where loss of daily loading cycles halts solute transport and impairs primary cilia-mediated mechanotransduction in osteoblasts~\cite{Liang_2024, Liu_2022, Prodanov_2012}. This triggers the ``Nuclear Stiffness Gauge'' downregulation (Active Inference failure) and stabilizes HIF-1$\alpha$, locking the spine in a ``permanent night'' swelling state that degrades the matrix ($B_{\mathrm{eff}}$) and precipitates scoliotic buckling.}
