...
% Figure 8: Lenke Classifications
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{fig_lenke_classes.png}
    \caption{\textbf{Lenke Curve Classifications as Cosserat Rod Eigenmodes.} Simulating the 6 Lenke curve types as dominant buckling modes by solving the generalized eigenvalue problem $(B y^{\prime\prime})^{\prime\prime} = \lambda Q y$. Shaded regions highlight localized metabolic energy deficits, representing regions where proportional proprioceptive delay $\tau(s)$ exceeds local damping $b(s)$ combined with regional stiffness alterations $B(s)$. The models accurately predict curve morphology characteristics observed clinically, including the common main thoracic pattern (Lenke Type 1) and complex patterns such as the triple major curve (Lenke Type 4).}
