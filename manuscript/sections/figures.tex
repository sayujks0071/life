...
\begin{figure}[h!]
    ...
    \caption{\textbf{The Allometric Trap and Biological Counter-Curvature.} (A)~The fundamental problem: a passive beam sags into a C-shape under gravity (blue), while the living spine acts as a \textbf{Thermodynamic Standing Wave}, actively maintaining an S-shaped counter-curvature (orange) at continuous metabolic cost $P_{\mathrm{counter}}$. This dynamic architecture functions as a sensorimotor control system minimizing the metric tensor deviation $\widehat{D}_{\mathrm{geo}}$ between the HOX-encoded target and structural reality. (B)~The Bio-Gravitational Number $\mathcal{B}_g = EI/(MgL^2)$ across vertebrate species. Species above the threshold ($\mathcal{B}_g > 0.1$, dashed line) are passively stable; humans ($\mathcal{B}_g \approx 0.01$) occupy the Allometric Trap, relying on constant cellular mechanotransduction (via Piezo1/2 and tensegrity networks) and active metabolic investment to prevent geometric collapse. (C)~The Energy Deficit Window: the metabolic cost of counter-curvature (red, scaling as $L^4$) outpaces the proprioceptive supply capacity (blue, scaling as $L^2$) beyond a critical length $L_{\mathrm{crit}} \approx 0.35$\,m. At $L = 0.45$\,m (adolescent spine), the deficit reaches ${\sim}41\%$. We propose this precipitates clinical Adolescent Idiopathic Scoliosis through the ``Derivative Gain Trap,'' where delayed sensory feedback destabilizes the inverted pendulum, driven by systemic energetic shortfall rather than localized genetic defects.}
