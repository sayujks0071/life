...
\begin{figure}[h!]
    ...
    \caption{\textbf{Synthetic-cohort calibration against published trial rates, not patient-level records.} (A)~In a simulated $N=1{,}000$ demographic draw, $B_g^{\mathrm{eff}}$ correlates with the Lonstein \& Carlson Progression Factor ($PF = (\Phi - 3R)/\mathrm{Age}$) because both are functions of the same sampled age/Cobb/Risser variables ($r = -0.483$). This is mathematical structure, not an independent clinical test. (B)~The model sigmoid was calibrated to the published BrAIST overall braced success rate. (C)~After that calibration, simulated observation-arm and Risser-stratum rates are compared with published BrAIST percentages. Patient-level BrAIST or Lonstein records are \emph{not} in this repository (see Figure~\ref{fig:spineweb_geometry} for the open-data geometry check that \emph{is} in-repo).}
    \label{fig:clinical_validation}
\end{figure}

% Figure 9: Open SpineWeb landmark geometry (real public dataset)
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{fig_spineweb_geometry_validation.png}
    \caption{\textbf{Open SpineWeb/AASCE landmark geometry (public dataset; not progression validation).} Derived Cobb-like endplate envelopes and AP centerline metrics from 609 curated T1--L5 landmark cases (534 usable/complete). Endplate envelope tracks tortuosity, lateral span, chord deviation, and projected-turn proxies in the expected direction. These Cobb-like values are computed from the same landmarks, not from independent clinician measurements, and the package has no Risser/Sanders stage, growth velocity, treatment exposure, or serial Cobb outcomes~\cite{wu2020spineweb}.}
    \label{fig:spineweb_geometry}
\end{figure}

% Figure 10: Literature Cobb anchors (n=5 aggregate points)
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{fig_clinical_cohort_data.png}
    \caption{\textbf{Literature-extracted Cobb anchors versus model $L_{\mathrm{crit}}$.} Five aggregate age--Cobb points digitized from published Weinstein (1983) and Lonstein (1984) figures/tables (\texttt{data/clinical\_cohort\_targets.csv}). These are not patient-level records. The vertical line is the model $L_{\mathrm{crit}}=0.35$\,m age correspondence. \textbf{Gap:} this repository does not contain a clinical Cobb series.}
    \label{fig:clinical_cohort_anchors}
\end{figure}

% Figure 11: PHV overlay
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{fig_phv_timing.png}
    \caption{\textbf{Model instability window versus published AIS onset windows.} The growth curve and deficit ratio are model output. Shaded onset windows are the Cheng (2015) female 10--13\,y and male 12--15\,y ranges already cited in \texttt{scripts/validate\_tau\_clinical.py}~\cite{cheng2015ais}. No measured peak-height-velocity trajectories are in this repository.}
    \label{fig:phv_overlay}
\end{figure}

% Figure 12: Sex-specific overlay
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{fig_sexual_dimorphism.png}
    \caption{\textbf{Sex-specific model windows versus published onset windows.} Female/male $R_{\mathrm{peak}}$ values are model parameters. Horizontal bars are Cheng (2015) onset windows already cited in-repo~\cite{cheng2015ais}. This repository has no sex-stratified patient cohort.}
    \label{fig:sex_overlay}
\end{figure}
