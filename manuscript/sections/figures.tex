...
\begin{figure}[h!]
    ...
    \caption{\textbf{Calibration and Consistency Checks on a Simulated Cohort.} (A)~Consistency check against the Lonstein \& Carlson (1984) progression index in a simulated cohort of $N=1,000$ ($r = -0.483$, Pearson; $\rho = -0.452$, Spearman). Both $B_g^{\mathrm{eff}}$ and the Progression Factor ($PF = (\Phi - 3R)/\mathrm{Age}$) derive from the same simulated variables, so no $p$-value is interpreted. (B)~The model-predicted risk sigmoid (red) compared with the clinical nomogram (green), transitioning at the calibrated threshold $B_{g,c} = 0.0180$ ($\lambda = 110.0$; $r = 0.462$ on the same simulated inputs). (C)~Calibration to the BrAIST (2013) randomized bracing trial, with bracing modeled as load-sharing that reduces gravitational loading by 37\%. The overall success rates (48\% observation, 72\% braced) were the grid-search calibration targets; the Risser subgroups were not targets and match in direction but not magnitude (model 70\% vs trial 64\% for Risser 0; 74\% vs 78\% for Risser 1--2). No patient records were used.}
