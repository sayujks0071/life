...
\begin{figure}[h!]
    ...
    \caption{\textbf{The Allometric Trap and Biological Counter-Curvature.} (A)~The fundamental problem: a passive beam sags into a C-shape under gravity (blue), while the living spine acts as an active \textbf{Thermodynamic Standing Wave}, maintaining an S-shaped counter-curvature (orange) at continuous metabolic cost $P_{\mathrm{counter}}$. This mechanobiological computation continuously minimizes the metric deviation $\widehat{D}_{\mathrm{geo}}$ between the HOX-encoded information field and gravity's vector. (B)~The Bio-Gravitational Number $\mathcal{B}_g = EI/(MgL^2)$ across species. Species above the threshold ($\mathcal{B}_g > 0.1$) are passively stable; humans ($\mathcal{B}_g \approx 0.01$) occupy the Allometric Trap, requiring constant mechanosensory feedback (via Piezo1/2) and active energy input to prevent collapse. (C)~The Energy Deficit Window: the metabolic cost of this counter-curvature (red, $L^4$) outpaces nutrient supply (blue, $L^2$) at length $L_{\mathrm{crit}} \approx 0.35$\,m. During the adolescent growth spurt ($L = 0.45$\,m), this transient deficit degrades the feedback loop, leading to the ``Derivative Gain Trap''—a delay-driven instability where active counter-curvature transitions into scoliotic buckling.}
