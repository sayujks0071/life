...
\begin{figure}[h!]
    ...
    \caption{\textbf{Gravity-Selected vs. Information-Selected Modes.} (A) First three eigenmodes of a passive uniform beam in gravity; the ground state (Mode 0, blue) is a monotonic C-shaped sag, representing the minimum potential energy state for a lifeless structure. (B) Eigenmodes of the IEC-coupled beam ($\chi_\kappa=0.05$); the shift in effective stiffness $B_{\mathrm{eff}}(s)$—representing the metabolically maintained ``Active Posterior''—reorders the spectrum, selecting the S-shaped mode as the new energetic ground state. This shift corresponds to the activation of the ``Counter-Curvature'' mechanism, where biological information ($I(s)$) overrides passive mechanics. (C) Normalized eigenvalue spectrum $\lambda_n/\lambda_0$ showing the phase transition in the lowest energy mode. This mode selection is physically implemented by the differential stiffening of the spinal column via the ``Mechanostat'' feedback loop, powered by the metabolic cost of maintaining cytoskeletal tension against gravity.}
