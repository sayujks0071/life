...
\begin{figure}[h!]
    ...
    \caption{\textbf{The Biological Counter-Curvature Mechanism as Active Inference.} (A) \textbf{Generative Model}: Discrete HOX gene expression domains (colored bars) define positional identity, which is smoothed into a continuous information field $I(s)$. This field specifies the \textbf{Reference Metric} ($\bar{g}$) of the tissue---the ``expectation'' of spinal shape. (B) \textbf{Sensory Evidence}: Gravity acts as the external force that generates the \textbf{Actual Metric} ($g$)---the physical shape of the spine. (C) \textbf{Prediction Error Minimization}: The system minimizes the free energy $U_{CC}$, defined as the divergence between expectation and reality ($\mathcal{E} = g - \bar{g}$). Mechanosensors act as error detectors: Vector Sensors (e.g., Piezo2) measure directional error, while Scalar Sensors (e.g., Piezo1) measure volumetric error. Active muscle tone and differential growth work to minimize $\mathcal{E}$, maintaining the stable S-shaped counter-curvature. In microgravity, the loss of Sensory Evidence (gravity) removes the ``tuning fork,'' causing the Generative Model to drift (Spinal Jetlag) and silencing Vector Sensors, leading to deformity.}
