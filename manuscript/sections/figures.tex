...
\begin{figure}[h!]
    ...
    \caption{\textbf{Thermodynamic Collapse: The Energy Deficit Window.} (A) The metabolic power $P_{\mathrm{counter}}$ (red) required to maintain the Counter-Curvature active standing wave against gravity scales with spinal length as $L^4$. (B) In contrast, the proprioceptive supply capacity (blue), governed by sublinear neural maturation and surface-limited diffusion, scales as $L^{2}$. The intersection at $L_{\mathrm{crit}} \approx 0.35$~m marks the onset of the \textbf{Energy Deficit Window} ($L > L_{\mathrm{crit}}$), resulting in a critical metabolic deficit at mid-adolescence ($L=0.45$~m). Structural analysis of \textbf{Thermodynamic Cost Proteins} (Table 2) reveals the molecular basis of this deficit: high-anisotropy sensors like Piezo2 and Lamin A/C (demand-side) are structurally expensive to maintain~\cite{lynch2015bioenergetic}, while compact supply-side enzymes (PLOD1, MMPs) are cheaper but rate-limited. In the deficit regime, the system cannot afford the high-fidelity state, triggering a \textbf{Convective Shutdown}~\cite{urban2004nutrition} and a ``Metabolic Buckling'' collapse into the low-energy scoliotic mode.}
