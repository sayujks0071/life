...
\begin{figure}[h!]
    ...
    \caption{\textbf{The Allometric Trap and Metabolic Buckling.} (A)~The physical problem: a passive beam sags into a C-shape under gravity (blue), whereas the living human spine functions as an active \textbf{Thermodynamic Standing Wave}. It maintains an S-shaped counter-curvature (orange) at a continuous metabolic cost ($P_{\mathrm{counter}}$) to minimize the metric deviation $\widehat{D}_{\mathrm{geo}}$ between the HOX-encoded information field and structural reality. (B)~The Bio-Gravitational Number $\mathcal{B}_g = EI/(MgL^2)$ across vertebrates. Small quadrupeds ($\mathcal{B}_g > 0.1$, dashed line) are passively stable, but humans ($\mathcal{B}_g \approx 0.01$) occupy a unique Allometric Trap, relying on constant, energy-demanding mechanosensory feedback (e.g., Piezo1/2, primary cilia) to prevent geometric collapse. (C)~The \textbf{Energy Deficit Window}: the thermodynamic cost of maintaining structural counter-curvature scales as $L^4$ (red), rapidly outpacing proprioceptive supply capacity which scales as $L^2$ (blue) past a critical length $L_{\mathrm{crit}} \approx 0.35$\,m. At $L = 0.45$\,m (the adolescent growth spurt), the energy deficit reaches ${\sim}41\%$. This metabolic shortfall exacerbates neural propagation delays, forcing the system into a destabilizing ``Derivative Gain Trap.'' Scoliosis thus emerges not as a local bone disease, but as a macroscopic buckling adaptation to localized metabolic insolvency.}
