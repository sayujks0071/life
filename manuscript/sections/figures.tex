...
\begin{figure}[h!]
    ...
    \caption{\textbf{The Biological Counter-Curvature Mechanism as Active Inference.} (A) \textbf{The Generative Model}: Discrete HOX gene expression domains encode a ``Genetic Prior'' ($\bar{g}$, blue curve) representing the evolutionarily optimized S-shape. This Top-Down prediction defines the target geometry. (B) \textbf{Sensory Evidence}: Gravity acts as the ``Interrogator,'' imposing a passive C-shaped sag (Actual Metric $g$). The discrepancy constitutes the Prediction Error ($U_{CC}$). (C) \textbf{The Vector-Scalar Mismatch}: In healthy gravity, Vector Sensors (Piezo2, Lamin A/C) and Scalar Sensors (Piezo1, TRPV4, SLYM) provide coherent error signals. In microgravity or AIS, Vector Sensors fail (silence due to unloading), while Scalar Sensors remain active or hyperactive due to osmotic swelling and fluid shifts. This dissociation blinds the system to orientation while signaling high metabolic stress, driving ``Spinal Jetlag''—a panic response of high-gain ``Congestive Remodeling'' where the system attempts to resolve a scalar error with gross geometric distortion (scoliosis).}
