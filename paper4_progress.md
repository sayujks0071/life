...
## Completed Sessions
* **Phase 1, Day 2 (Literature Review — AIS genetics)** [2024-05-15]: Completed. Reviewed the major GWAS hits for AIS, including LBX1, GPR126 (ADGRG6), PAX1, BNC2, and CHL1. Mapped these genetic loci to their proposed biological functions and explicitly categorized them based on their impact on the PID inverted pendulum model (i.e., whether they alter the controller via $\tau$ or the structural "plant"). LBX1 and GPR126 emerge as the strongest candidates for modulating $\tau_{spin}$ and $\tau_{aff}$, respectively. Output generated in `paper4_literature/day2_ais_gwas.md`.
...
* AIS genetics supports a dual-mechanism etiology: some genes (PAX1, BNC2) modify the biomechanical "plant" (vertebral development), while others (LBX1, GPR126) degrade the feedback controller by increasing components of the proprioceptive delay $\tau$.
