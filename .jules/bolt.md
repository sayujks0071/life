...
## $(date +%Y-%m-%d) - Optimize Radius of Gyration memory allocation
**Learning:** For computing the radius of gyration, allocating an intermediate N x 3 squared distance array is a significant memory and time bottleneck for large coordinate structures. We can leverage the identity that the mean squared distance from the center of mass equals the sum of the variances of the individual coordinates (`np.sum(np.var(coords, axis=0))`). The PAE domain extraction optimization attempt (using `np.ix_` and `dtype=np.uint32`) failed because `np.add.reduceat` silently truncated the floating point values leading to precision loss, and `np.ix_` introduced overhead compared to explicit array indexing.
**Action:** Replaced `np.sum((coords - center_of_mass)**2, axis=1)` with `np.sum(np.var(coords, axis=0))` in `MetricsAnalyzer.calculate_rg` to calculate the variance directly, achieving a substantial execution speedup and reducing peak RAM usage without affecting scientific outputs. Also removed the incorrect PAE matrix optimization.
