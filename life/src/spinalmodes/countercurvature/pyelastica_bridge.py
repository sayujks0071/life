...
- Optional localized, time-varying cilia moments superposed on M_info
...
class CounterCurvatureRodSystem:
    ...
    def __init__(
        ...
        *,
        ...
        params:
            Countercurvature coupling parameters.
        cilia:
            Optional cilia activation pattern for localized active moments.
        """
        _check_pyelastica()
        self.rod = rod
        self.info_field = info_field
        self.params = params
        self.cilia_pattern = cilia
    ...
    def from_iec(
        ...
        *,
        ...
        cilia:
            Optional cilia activation pattern supplying localized, time-varying
            active moments and metric perturbations.
        ...
        M_info = compute_active_moments_with_cilia(info, params, cilia=cilia)
        ...
        if cilia is not None:
            rod._cilia_pattern = cilia  # type: ignore

        return cls(rod=rod, info_field=info, params=params, cilia=cilia)
    ...
    def run_simulation(
        ...
        *,
        ...
        - Optional cilia-driven moments if a CiliaPattern is provided
        ...
        # Add active moments from information gradients and cilia actuation
        cilia_pattern = getattr(self.rod, "_cilia_pattern", None)
        if self.params.chi_M != 0.0 or cilia_pattern is not None:
            M_info = getattr(self.rod, "_countercurvature_M_info", None)  # type: ignore

            class ActiveMomentForcing(ea.NoForces):
                """Apply information-driven and cilia-driven moments as body couples."""

                def __init__(
                    self,
                    M_info: ArrayF64 | None,
                    rod: "ea.CosseratRod",  # type: ignore
                    cilia_pattern: CiliaPattern | None = None,
                ):
                    super().__init__()
                    self.M_info = M_info
                    self.cilia_pattern = cilia_pattern
                    self.rod = rod
                    self.s_cumsum = np.concatenate([[0.0], np.cumsum(rod.rest_lengths)])
                    self.s_elements = self.s_cumsum[:-1] + 0.5 * self.rod.rest_lengths

                def _interp(self, values: ArrayF64) -> ArrayF64:
                    return np.interp(self.s_elements, self.s_cumsum[:-1], values[:-1])

                def apply_forces(self, system, time: float = 0.0):
                    """Apply active moments as internal couples about y-axis (sagittal bending)."""
                    M_interp = np.zeros_like(self.s_elements)
                    if self.M_info is not None:
                        M_interp += self._interp(self.M_info)
                    if self.cilia_pattern is not None:
                        M_interp += self.cilia_pattern.evaluate(self.s_elements, time=time)
            ...
            system.add_forcing_to(self.rod).using(
                ActiveMomentForcing, M_info=M_info, rod=self.rod, cilia_pattern=cilia_pattern
            )
