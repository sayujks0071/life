...
class CiliaPattern:
    """Localized, possibly oscillatory cilia activation profile."""
    ...
    def _envelope(self, s: ArrayF64) -> ArrayF64:
        if self.width <= 0.0:
            raise ValueError("CiliaPattern.width must be positive.")
        env = np.zeros_like(s, dtype=float)
        for center in self.centers:
            env += np.exp(-((s - center) ** 2) / (2.0 * self.width**2))
        return env

    def evaluate(self, s: ArrayF64, time: float = 0.0) -> ArrayF64:
        """Return a signed active moment field from beating cilia."""
        envelope = self._envelope(s)
        oscillation = np.cos(2.0 * np.pi * self.frequency * time + self.phase)
        return self.direction * self.amplitude * envelope * oscillation
...
def compute_cilia_active_moments(
    info: InfoField1D, cilia: CiliaPattern, *, time: float = 0.0
) -> ArrayF64:
    """Active moment field contributed by localized cilia beats."""

    return cilia.evaluate(info.s, time=time)


def compute_active_moments_with_cilia(
    info: InfoField1D,
    ...
    *,
    ...
    """Composite active moments from information gradients and cilia actuation."""

    base = compute_active_moments(info, params)
    if cilia is None:
        return base
    return base + compute_cilia_active_moments(info, cilia, time=time)
