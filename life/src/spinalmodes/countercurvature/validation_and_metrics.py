...
from .coupling import CiliaPattern
...
def modulate_metric_with_cilia(
    g_eff: ArrayF64, info: InfoField1D, cilia: CiliaPattern, *, beta_cilia: float = 0.2
) -> ArrayF64:
    """Boost g_eff in regions where cilia beat to capture metric perturbations."""

    if g_eff.shape != info.s.shape:
        raise ValueError("g_eff must match the information grid.")
    envelope = np.abs(cilia.evaluate(info.s, time=0.0))
    if envelope.max() > 0:
        envelope = envelope / float(np.max(envelope))
    return g_eff * (1.0 + beta_cilia * cilia.metric_boost * envelope)
