...
from spinalmodes.experiments.countercurvature.common import create_cilia_pattern
...
def run_microgravity_experiment(
    ...
    cilia:
        Optional cilia pattern that adds localized active moments and boosts the
        countercurvature metric near genetic asymmetries.
    ...
    if cilia is not None:
        g_eff = modulate_metric_with_cilia(g_eff, info_field, cilia, beta_cilia=0.3)
    ...
    M_info = compute_active_moments_with_cilia(info_field, params, cilia=cilia)
    ...
    for g, label, gravity_load in zip(gravity_levels, gravity_labels, gravity_loads):
        ...
        # IEC energy functional (Eq. 2 analogue)
        rhoA_local = gravity_load / max(g, 1e-12)
        iec_energy = information_elasticity_energy(
            info_field,
            kappa,
            bending_stiffness=E_eff * I_moment,
            kappa_reference=kappa_passive,
            rhoA=rhoA_local,
            gravity=g,
            k_info=chi_kappa,
            metric=g_eff,
        ...
        # Standing-wave dispersion relation
        dispersion = standing_wave_dispersion_relation(
            kappa, info_field, rho=rhoA_local, beta=chi_kappa
        )
        ...
        result = {
            ...
            "iec_energy_total": iec_energy["total"],
            "iec_energy_info": iec_energy["info_energy"],
            "iec_energy_gravity": iec_energy["gravity_energy"],
            "iec_energy_bending": iec_energy["bending_energy"],
            "omega_mean": dispersion["omega_mean"],
            "omega_peak": dispersion["omega_peak"],
        ...
        summary_rows.append(
            {
                ...
                "iec_energy_total": result["iec_energy_total"],
                "iec_energy_info": result["iec_energy_info"],
                "iec_energy_gravity": result["iec_energy_gravity"],
                "iec_energy_bending": result["iec_energy_bending"],
                "omega_mean": result["omega_mean"],
                "omega_peak": result["omega_peak"],
                "with_cilia": bool(cilia is not None),
...
def main():
    ...
    parser.add_argument(
        "--with-cilia",
        action="store_true",
        help="Include a cilia-driven active moment patch in the thoracic region.",
    )
    ...
    cilia = create_cilia_pattern(
        length,
        center=0.6,
        width=0.04 if args.quick else 0.03,
        amplitude=0.03,
        frequency=1.5,
        metric_boost=0.7,
    ) if args.with_cilia else None
    ...
    if args.with_cilia:
        print("   Including thoracic cilia patch (active moments + metric boost)")
    print()
    ...
    print()
    print("-" * 80)
    print(f"{'Gravity':<15} | {'D_geo_norm':<12} | {'Preservation':<12} | {'ω_mean':<10} | {'E_IEC':<10}")
    print("-" * 80)
    for result in results["results"]:
        print(
            f"{result['gravity_label']:<15} | {result['D_geo_norm']:>10.6f}   | "
            f"{result['shape_preservation_index']:>10.3f} | "
            f"{result['omega_mean']:>8.3f} | {result['iec_energy_total']:>9.2e}"
        )
    print("-" * 80)
    print()
