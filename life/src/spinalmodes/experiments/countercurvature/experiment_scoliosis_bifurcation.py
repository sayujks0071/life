...
New in this version:
- Eigenproblem (Eq. 4 analogue) tracks the instability threshold λ₀.
- Optional cilia patch can add localized active moments/metric boosts.
...
from spinalmodes.countercurvature.validation_and_metrics import modulate_metric_with_cilia
...
def run_scoliosis_bifurcation_experiment(
    ...
    output_dir:
        Output directory.
    cilia:
        Optional cilia-driven active moment patch to test cilia–asymmetry interactions.
    ...
    rhoA = gravity_load / 9.81  # approximate linear density from load
    ...
    for chi_k in chi_kappa_values:
        for eps_asym in asymmetry_values:
            ...
            # Eigenproblem (instability threshold)
            eigen_result = solve_countercurvature_eigenproblem(
                info_field,
                params,
                E0=E0,
                I_moment=I_moment,
            ...
            bifurcation_data.append({
                ...
                "lambda0": eigen_result["critical_eigenvalue"],
                "unstable": eigen_result["unstable"],
    ...
    Z = pivot.values
    lambda_pivot = df.pivot(index="epsilon_asym", columns="chi_kappa", values="lambda0")
    lambda_vals = lambda_pivot.loc[pivot.index, pivot.columns].values
    ...
    ax.contour(X, Y * 100, lambda_vals, levels=[0.0], colors="cyan", linewidths=2, linestyles="-.")
    ...
    proxy_unstable = Line2D([0], [0], color="cyan", linestyle="-.", label="λ₀ = 0 (eigen threshold)")
    ax.legend(handles=[proxy_unstable], fontsize=9)
