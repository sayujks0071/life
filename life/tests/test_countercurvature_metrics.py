...
from spinalmodes.countercurvature.iec_analysis import (
...
def test_information_elasticity_energy_components():
    """IEC energy should capture gradient + bending contributions."""
    s = np.linspace(0, 1.0, 50)
    I = np.sin(np.pi * s)
    dIds = np.gradient(I, s)
    info = InfoField1D(s=s, I=I, dIds=dIds)
    kappa = 0.1 * np.ones_like(s)

    energies = information_elasticity_energy(
        info,
        kappa,
        bending_stiffness=1.0,
        kappa_reference=np.zeros_like(s),
        rhoA=1.0,
        gravity=9.81,
        k_info=0.5,
    )

    assert energies["info_energy"] > 0.0
    assert energies["total"] > energies["info_energy"]
    assert "gravity_energy" in energies and "bending_energy" in energies


def test_countercurvature_eigenproblem_stability_flag():
    """Eigenproblem should return ordered eigenvalues and stability flag."""
    length = 0.4
    s = np.linspace(0, length, 60)
    I = 0.3 + 0.2 * np.sin(2 * np.pi * s / length)
    dIds = np.gradient(I, s)
    info = InfoField1D(s=s, I=I, dIds=dIds)
    params = CounterCurvatureParams(chi_kappa=0.02, chi_E=0.0, chi_M=0.0, scale_length=length)

    eig = solve_countercurvature_eigenproblem(
        info,
        params,
        E0=1e8,
        I_moment=1e-8,
    ...
    assert len(eig["eigenvalues"]) == 3
    assert eig["critical_eigenvalue"] == pytest.approx(eig["eigenvalues"][0])
    assert np.all(np.isfinite(eig["eigenvalues"]))
