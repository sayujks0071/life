...
def solve_multi_segment_cosserat_buckling(N=100, lenke_type=1, polygenic_multiplier=1.0):
    """
    Generalized 1D buckling eigenvalue problem (B y'')'' = \\lambda Q y
    for a multi-segment Cosserat rod.

    Predicts Lenke curve types (1-6) using regional parameter profiles.
    polygenic_multiplier scales the base instability drive Q.
    """
    # Normalized spine length (z from 0 to 1)
    z = np.linspace(0, 1, N)
    dz = z[1] - z[0]

    # Base parameters
    B = np.ones(N) * 1.0  # Stiffness
    Q = np.ones(N) * 0.1 * polygenic_multiplier  # Instability drive
