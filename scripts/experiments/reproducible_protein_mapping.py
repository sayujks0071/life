"""
Reproducible Protein-to-Geometry Mapping Experiment.

This script maps protein/ECM-inspired parameters (specifically FBN1-mediated stiffness anisotropy
and growth signaling gain) to emergent mechanical outputs (curvature, torsion) using a
PyElastica-based Cosserat rod simulation.

It serves as a minimal, reproducible experiment for the "Biological Countercurvature" pipeline.

Usage:
    python scripts/experiments/reproducible_protein_mapping.py --scenario bio_map
    python scripts/experiments/reproducible_protein_mapping.py --quick-test
"""
...
def run_experiment(
    ...
    with open(out_file, mode='a', newline='') as csvfile:
        ...
        print("-" * 120)
        print(
            f"{'Bio Label':<40} | {'Cobb (deg)':<10} | {'Max Curv':<10} | {'Energy (J)':<10} | {'Time (s)':<8}"
        )
        print("-" * 120)
...
def generate_markdown_report(csv_file: str, results: List[Dict[str, Any]]):
    ...
    with open(md_file, "w") as f:
        ...
        f.write("| Bio Label | Anisotropy | Active Curv | Cobb Angle (deg) | Max Curvature | Energy (J) |\n")
        f.write("|---|---|---|---|---|---|\n")

        for r in results:
            f.write(
                f"| {r['bio_label']} | {r['input_anisotropy']:.2f} | {r['input_active_curvature']:.2f} | "
                f"{r['cobb_angle']:.4f} | {r['max_curvature']:.4f} | {r['bending_energy']:.4e} |\n"
            )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Reproducible Protein Mapping Experiment")

    parser.add_argument(
        "--out-file",
        type=str,
        default="outputs/protein_mapping_results.csv",
