...
def main():
    ...
    for l_type in range(1, 7):
        # We use solve_multi_segment_cosserat_buckling to predict Lenke curve 1-6
        z, evalue, evec, B, Q = solve_multi_segment_cosserat_buckling(N=100, lenke_type=l_type)

        full_mode = np.zeros(100)
        full_mode[2:-2] = evec
        # Normalize amplitude to 1 for easier interpretation of shape
        if np.max(np.abs(full_mode)) > 1e-8:
