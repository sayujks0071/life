...
def run_phase_diagram():
    print("Running Phase Diagram (tau vs L)...")
    lengths = np.linspace(0.2, 1.2, 20)
    delays = np.linspace(0.01, 0.1, 20) # v_nerve ~ 20 to 200 => tau = 2L/v

    stability_map = np.zeros((len(delays), len(lengths)))

    # We override v_nerve to directly set tau
    for i, tau in enumerate(delays):
        for j, L in enumerate(lengths):
            # Hack: set v_nerve to match tau = 2L/v_nerve => v_nerve = 2L/tau
            v_nerve = 2 * L / tau
            controller = DelayedSpineController(K_p=15.0, K_d=2.0, v_nerve=v_nerve)

            # Run
            t, theta, _ = controller.run_simulation(L, T_max=5.0)

            # Check stability
            last_indices = int(1.0 / 0.001)
            amplitude = np.max(np.abs(theta[-last_indices:]))

            is_stable = amplitude < 0.1
            stability_map[i, j] = 1 if is_stable else 0

    plt.figure(figsize=(10, 8))
    plt.imshow(stability_map, origin='lower',
               extent=[lengths.min(), lengths.max(), delays.min(), delays.max()],
               aspect='auto', cmap='RdYlGn')

    plt.colorbar(label='Stability (1=Stable, 0=Unstable)')
    plt.xlabel('Spine Length L (m)')
    plt.ylabel('Delay $\\tau$ (s)')
    plt.title('Information-Coupled Thermostatic Column\nStability Phase Diagram')

    output_path = f"{OUTPUT_DIR}/delay_phase_diagram.png"
