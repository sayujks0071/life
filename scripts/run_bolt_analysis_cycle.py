...
def main():
    ...
    # Initialize figures outside loops to avoid overhead
    ...
    for symbol in interesting_symbols:
        if symbol in pae_data:
            ...
            # Only add colorbar once, to avoid duplicate axes
            if not getattr(fig_pae, '_has_colorbar', False):
                fig_pae.colorbar(im_pae, ax=ax_pae, label='Expected Position Error (Å)')
                fig_pae._has_colorbar = True
