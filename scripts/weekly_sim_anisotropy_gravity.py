...
def run_sweep():
    ...
    # Setup parameters
    anisotropy_values = np.linspace(1.0, 5.0, 20)

    # Fixed parameters
    active_curvature = 14.0 # High growth
    ...
    print(f"Starting Anisotropy Sweep under Gravity (N={len(anisotropy_values)})...")
    print(f"Active Curvature: {active_curvature}, Gravity: {gravity}, Defect: {initial_lateral_defect}")

    out_dir = Path("outputs/sim/2026-04-02_anisotropy_gravity")
...
def generate_plots(df, out_dir):
    ...
    plt.title('Effect of Stiffness Anisotropy on Lateral Instability\n(Active Curvature=14.0, Gravity=9.81)')
