...
def run_sweep():
    ...
    # Sweep natural kyphosis
    kyphosis_values = np.linspace(0.0, 5.0, 16)
    anisotropy_val = 3.0
    active_curvature_val = 10.0 # High growth

    results = []

    # Run the sweep
    for ky in kyphosis_values:
        print(f"Running simulation with natural_kyphosis = {ky:.2f}...")
        ...
        config = {
            ...
            "initial_lateral_defect": 0.05, # Small defect to trigger instability
            "natural_kyphosis": ky,
            "length": 0.4,
            "n_elements": 50,
            "duration": 2.0,
    ...
    params_df = pd.DataFrame({"natural_kyphosis": kyphosis_values, "active_curvature": [active_curvature_val]*len(kyphosis_values), "anisotropy": [anisotropy_val]*len(kyphosis_values), "seed": [seed]*len(kyphosis_values)})
    ...
    plt.tight_layout()
    plt.savefig(out_dir / "plot_metrics_kyphosis.png", dpi=300)
    plt.close()
