...
def ensure_annotations_extracted() -> None:
    """Unpack the curated annotation zips if XML directories are missing."""
    for split in ("train", "test"):
        annotation_dir = DATA_DIR / f"annotations_{split}"
        zip_path = DATA_DIR / f"annotations_{split}.zip"
        existing = list(annotation_dir.glob("*.xml")) if annotation_dir.exists() else []
        if existing:
            continue
        if not zip_path.exists():
            raise FileNotFoundError(
                f"Missing SpineWeb annotations for split '{split}'. "
                f"Expected XML directory {annotation_dir} or zip {zip_path}."
            )
        annotation_dir.mkdir(parents=True, exist_ok=True)
        with zipfile.ZipFile(zip_path) as archive:
            archive.extractall(annotation_dir)
...
def main() -> None:
    ...
    manuscript_copy = FIGURES_DIR / "fig_spineweb_landmark_features.png"
    manuscript_copy.write_bytes(png_path.read_bytes())
