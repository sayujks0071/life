...
%% Custom shorthand macros (normalized deviation from passive gravitational sag)
\newcommand{\Dgeohat}{\widehat{D}_{\mathrm{shape}}}
...
To understand how the spine maintains its shape and why it is uniquely vulnerable during growth, we frame sagittal profile maintenance as an \textbf{Information--Elasticity Coupling (IEC)} problem: developmental patterning and proprioceptive feedback jointly set tissue-level reference curvatures and stiffness, producing the clinically observed S-shaped profile as a maintained, dissipative structure rather than a passive sag under gravity alone. The vertebrate spine functions as a \textbf{Thermodynamic Standing Wave}~\cite{prigogine1977self}, actively maintaining its lordotic--kyphotic geometry via continuous proprioceptive feedback and the investment of metabolic free energy~\cite{friston2013life}.
...
1.  \textbf{Shape deviation from passive sag} $\widehat{D}_{\mathrm{shape}}$: Quantifies the difference between the realized shape and the passive gravity-only equilibrium (C-shaped sag) in the same model.
...
Regimes are classified based on the normalized shape deviation $\widehat{D}_{\mathrm{shape}}$ (denoted $\Dgeohat$ in Results). We define the \emph{gravity-dominated} regime ($\widehat{D}_{\mathrm{shape}} < 0.1$) as the region where gravitational potential energy dominates, leading to monotonic sag. The \emph{cooperative} regime ($0.1 \leq \widehat{D}_{\mathrm{shape}} \leq 0.3$) corresponds to the range where patterning-driven IEC and gravity balance to produce a stable sagittal S-curve. The \emph{information-dominated} regime ($\widehat{D}_{\mathrm{shape}} > 0.3$) is reached when IEC-driven changes in rest curvature and stiffness dominate equilibrium geometry, which, as we show, also coincides with the onset of symmetry-breaking instabilities (scoliosis-like modes). These thresholds were determined by analyzing the bifurcation of the lowest eigenvalue $\lambda_0$ and the emergence of non-monotonic curvature profiles.
